...
def generate_slots(availability, bookings, selected_date: str, duration_hours):
    ...
    requested_day = requested_date.strftime("%A").lower()

    for block in availability:

        if block["day_of_week"] != requested_day:
            continue
        ...
        while current + duration <= availability_end:
            ...
            if not conflict:available_slots.append({"start_time": current,"end_time": slot_end,})
